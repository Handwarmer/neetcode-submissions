class Solution {
    public double findMedianSortedArrays(int[] nums1, int[] nums2) {
        ...
        while (l <= r) {
            ...
            int aL = (i == 0) ? Integer.MIN_VALUE : A[i-1]; //A的左半最大值
            int aR = (i == AL) ? Integer.MAX_VALUE : A[i]; //A的右半最小值
            int bL = (j == 0) ? Integer.MIN_VALUE : B[j-1];//B的左半最大值
            int bR = (j == BL) ? Integer.MAX_VALUE : B[j];//B的右半最小值
            if (Math.max(aL, bL) <= Math.min(aR, bR)) { //分割正确
                if ((AL + BL) % 2 == 1) return Math.max(aL, bL);
                return (Math.max(aL, bL) + Math.min(aR, bR)) / 2.0;
            } else if (aL > bR){ //A切多了，i左移
                r = i - 1;
            } else {
                l = i + 1;
            } 
